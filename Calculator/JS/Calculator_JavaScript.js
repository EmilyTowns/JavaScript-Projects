...
// this modifies values each time a botton is clicked
...
function Input_Decimal(dot) {
    // this ensures that accidental clicking of the decimal point doesn't casue bugs in your operation
    if (Calculator.Wait_Second_Operand === true) return;
    if (!Calculator.Display_Value.includes(dot)) {
        //we are saying that if the Display_Value does not contain a decimel point we want to add a decimal point
...
function Handle_Operator(Next_Operator) {
    ...
    // checks if and operator already exists and it Wait_Second_Operand is true, then updates the operator and exits from the functiom
